...
from smi.adapters.img_provider import load_img_data_url
...
async def main() -> None:
    ...
    image_data_url = load_img_data_url(sheet_metal_id)
    logger.info("Bild für %s geladen.", sheet_metal_id)

    results = await assess_sheet_metal(
        sheet_metal_id=sheet_metal_id,
        potential_ids=optimisation_potential_ids,
